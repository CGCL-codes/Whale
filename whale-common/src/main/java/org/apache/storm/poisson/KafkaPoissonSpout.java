...
public class KafkaPoissonSpout<K, V> extends BaseRichSpout {
    ...
    public KafkaPoissonSpout(KafkaSpoutConfig<K, V> kafkaSpoutConfig, int expect) {
        this(kafkaSpoutConfig, new KafkaConsumerFactoryDefault<>(), expect);
    }

    @VisibleForTesting
    public KafkaPoissonSpout(KafkaSpoutConfig<K, V> kafkaSpoutConfig, KafkaConsumerFactory<K, V> kafkaConsumerFactory, int expect) {
...
//                }
            ...
            while (start + 1_000_000_000 >= System.nanoTime()) {
                ;
            }
