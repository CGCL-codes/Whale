...
public class KafkaPoissonIntervalSpout<K, V> extends BaseRichSpout {
    ...
    protected ConcurrentHashMap<KafkaSpoutMessageId, Long> latencyHashMap; //用来记录tuple的msgID，和latency

    public KafkaPoissonIntervalSpout(KafkaSpoutConfig<K, V> kafkaSpoutConfig, int expect) {
        this(kafkaSpoutConfig, new KafkaConsumerFactoryDefault<>(), expect);
    }

    @VisibleForTesting
    public KafkaPoissonIntervalSpout(KafkaSpoutConfig<K, V> kafkaSpoutConfig, KafkaConsumerFactory<K, V> kafkaConsumerFactory, int expect) {
        this.kafkaConsumerFactory = kafkaConsumerFactory;
        this.kafkaSpoutConfig = kafkaSpoutConfig;
        this.expect = expect;
    }

    public void setCounter(Runnable counter) {
...
//                + "Defaulting to behavior compatible with earlier version", committedOffset);
    ...
    public void nextTuple() {
        try {
            ...
            if (emitIfWaitingNotEmitted()) {
                if (counter != null) counter.run();
            }
//                if (start + 1_000_000_000 < System.nanoTime()) {
//                    break;
//                }
            while (start + exp >= System.nanoTime()) {
                ;
            }
    ...
    // ======== emit  =========
    private boolean emitIfWaitingNotEmitted() {
            ...
            if (isEmitTuple(tuple)) {
                ...
                if (!isScheduled || retryService.isReady(msgId)) {
                    ...
                    if (!isAtLeastOnceProcessing()) {
                        ...
                        emitted.add(msgId);
                        //add tuple transfer Latency
                        latencyHashMap.put(msgId, System.currentTimeMillis());
    ...
    private void commitOffsetsForAckedTuples(Set<TopicPartition> assignedPartitions) {
        ...
        for (Entry<TopicPartition, OffsetManager> tpOffset : assignedOffsetManagers.entrySet()) {
