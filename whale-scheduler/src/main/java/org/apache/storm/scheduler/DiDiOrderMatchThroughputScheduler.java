 ...
 * Created by mastertj on 2018/7/15.
 */
public class DiDiOrderMatchThroughputScheduler implements IScheduler {
    ...
    private static Map<ExecutorDetails, WorkerSlot> scheduleTopology(TopologyDetails topology, Cluster cluster) {
        //SpoutExecutor
        ExecutorDetails spoutExecutor = topology.getComponents().get("kafka-spout").getExecs().get(0);
        boolean m_bool=false;

        List<WorkerSlot> availableSlots = cluster.getAvailableSlots();
        Set<ExecutorDetails> allExecutors = topology.getExecutors();
        allExecutors.remove(spoutExecutor);
        ...
        List<WorkerSlot> reassignSlots = sortedList.subList(1, toIndex);
        ...
        if(!aliveExecutors.contains(spoutExecutor)){
            m_bool=true;
        }
        ...
        //分配给SpoutExecutor的WorkSlot
        WorkerSlot slot = sortedList.get(0);
        ...
        if(m_bool)
            reassignment.put(spoutExecutor,slot);
