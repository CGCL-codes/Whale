...
public class SpoutExecutor extends Executor {
    ...
    //whale-multicast
    private final boolean nonblockMultiEnabled;
    private Timer queueMonitor;
    private int queueMonitorPeriod = 200; //streamMonitor的监控周期为200ms
    ...
    public SpoutExecutor(final WorkerState workerData, final List<Long> executorId, Map<String, String> credentials) {
        ...
        this.nonblockMultiEnabled = ObjectReader.getBoolean(topoConf.get(Config.TOPOLOGY_NONBLOCKMULIT_ENABLE), false);
    ...
    public void init(final Map<Integer, Task> idToTask) {
        ...
        for (Map.Entry<Integer, Task> entry : idToTask.entrySet()) {
            ...
            ISpoutOutputCollector spoutOutputCollector = null;
            if(this.nonblockMultiEnabled){
                ...
                spoutOutputCollector = new WhaleSpoutOutputCollectorImpl(
                        spoutObject, this, taskData, entry.getKey(), emittedCount,
                        hasAckers, rand, hasEventLoggers, isDebug, pending);
            }
        ...
        //whale-multicast
        if(this.nonblockMultiEnabled){
            this.queueMonitor=new Timer();
            this.queueMonitor.scheduleAtFixedRate(new TimerTask() {
                public void run() {
                    long currentLength = sendQueueAllGrouping.getMetrics().population();
                    BalancedPartialMulticastGraph<String> currentMulticastTree = (BalancedPartialMulticastGraph<String>) conf.get("nonblockMultiTree");
                    String currentStatus = MulticastController.determineSelfAdjustment(previousLength, currentLength);
                    if(currentStatus!=null){
                        int capacity = ObjectReader.getInt(topoConf.get(Config.TOPOLOGY_DISRUPTOR_BATCH_SIZE));
                        int outDegree = currentMulticastTree.getMaxDegree();
                        //考虑源头节点多条流输入的情况
                        int newDegree = MulticastController.computeMaxOutDegree(outputCollectors.get(0).getInputRate(),capacity);
            ...
            }, 2000,queueMonitorPeriod);// 设定指定的时间time,此处为queueMonitorPeriod毫秒
        }
    }
